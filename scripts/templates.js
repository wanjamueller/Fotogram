function framesTemplate(i) {
    ...
    <figure class="picture">
        <img id="pic${i}"  src="./assets/img/${myImages[i].file}" tabindex="0"  aria-controls= "image-dialog" onclick="focusPicture(${i})" onkeypress="focusPicture(${i})" alt="${myImages[i].alt}">;
    </figure>`;
}

function dialogTemplate(i) {
    return /*html*/ `
    <div onclick="eventBubblingPrevention(event)" aria-labelledby="dialogTitle" class="dialog">
        <header class="dialog-header">
            ...
            <figure class="focus-picture"><img class="picture-open" id="pic${i}" src="./assets/img/${myImages[i].file}" alt="${myImages[i].alt}" onclick="focusPicture(${i})"></figure>
